...
const SITE_URL = (process.env.SITE_URL || 'https://pdfbay.pages.dev').replace(
  /\/+$/,
  ''
);
...
function injectOrganizationLd(document) {
  ...
  const data = {
    ...
    sameAs: ['https://github.com/DevRuibin/pdfbay'],
  };
...
const BRAND_NAME = process.env.VITE_BRAND_NAME || 'PDFBay';
  ...
  if (tools[translationKey]) {
    title =
      ...
      (tools[translationKey].name
        ? `${tools[translationKey].name} - PDFBay`
        : null);
